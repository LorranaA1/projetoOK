  ...
  Future<void> onPressedButton() async {
    ...
    if (_formVideoKey.currentState!.validate()) {
      ...
      if(listaContent.isNotEmpty){
        print('ta entrando');
        listaVideo = await VideosDao().getLastId(listaContent[0].id);
        print('ta saindo');
        if(listaVideo[0].id!=null){
          print(listaVideo[0].id);
      ...
      print(id_new);
      if(listaContent.isNotEmpty){
        Videos new_video = Videos(id: id_new, urlImage: urlImage,
            tituloVideo: tituloVideo, tempoDuracao: tempoDuracao,
            urlVideo: urlVideo,
            idContent: listaContent[0].id);
        await VideosDao().addVideo(videos: new_video);
